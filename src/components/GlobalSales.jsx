...
function FilterBar({ search, setSearch, seller, setSeller, category, setCategory, sellers, categories }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <select value={seller} onChange={e => setSeller(e.target.value)} style={{ flex: 1, fontSize: 13 }}>
          <option value="">Todos los vendedores</option>
          {sellers.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={category} onChange={e => setCategory(e.target.value)} style={{ flex: 1, fontSize: 13 }}>
...
function SalesView({ title, subtitle, sales, loading, emptyIcon, emptyTitle, emptySub }) {
  ...
  const filtered = useMemo(() => sales.filter(s => {
    ...
    const matchSearch = !q || s.product?.toLowerCase().includes(q) || s.notes?.toLowerCase().includes(q) || s.location?.toLowerCase().includes(q);
    const matchSeller = !seller || s.sellerName === seller;
    const matchCat = !category || s.category === category;
    return matchSearch && matchSeller && matchCat;
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: 18, fontWeight: 600, marginBottom: 2 }}>{title}</h1>
        <p style={{ fontSize: 13, color: "var(--gray-500)" }}>{subtitle}</p>
      </div>

      {/* Stats rápidas */}
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <div style={{ flex: 1, background: "#fff", border: "1px solid var(--gray-100)", borderRadius: "var(--radius-md)", padding: "10px 14px" }}>
          <div style={{ fontSize: 11, color: "var(--gray-500)" }}>Total {hasFilters ? "filtrado" : ""}</div>
          <div style={{ fontSize: 18, fontWeight: 600, color: "var(--teal)" }}>{fmx(total)}</div>
        </div>
        <div style={{ flex: 1, background: "#fff", border: "1px solid var(--gray-100)", borderRadius: "var(--radius-md)", padding: "10px 14px" }}>
          <div style={{ fontSize: 11, color: "var(--gray-500)" }}>Ventas</div>
          <div style={{ fontSize: 18, fontWeight: 600 }}>{filtered.length}</div>
        </div>
      </div>
      ...
      {hasFilters && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
          ...
          <span onClick={() => { setSearch(""); setSeller(""); setCategory(""); }} style={{ fontSize: 11, padding: "3px 10px", background: "var(--gray-100)", color: "var(--gray-500)", borderRadius: 20, cursor: "pointer" }}>Limpiar todo</span>
        </div>
      )}
...
export function GlobalSales() {
  ...
  return (
    <SalesView
      ...
      subtitle="Todas las ventas públicas"
      sales={sales}
      loading={loading}
      emptyIcon="🌐"
      emptyTitle="Sin ventas públicas"
      emptySub="Registra una venta con visibilidad pública"
    />
  );
}
...
export function PrivateSales() {
  ...
  return (
    <SalesView
      ...
      subtitle="Solo tú puedes verlas"
      sales={sales}
      loading={loading}
      emptyIcon="🔒"
