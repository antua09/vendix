...
function HomeIcon({ active }) {
  const c = active ? "var(--teal)" : "var(--text3)";
...
function ChatIcon({ active }) {
  const c = active ? "var(--teal)" : "var(--text3)";
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>;
}
...
function Avatar({ user, size = 28 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: "var(--teal-light)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.4, color: "var(--teal-dark)", fontWeight: 600, flexShrink: 0 }}>
    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <aside className="desktop-sidebar" style={{ width: 220, background: "var(--surface)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", padding: "1.25rem 1rem", flexShrink: 0, height: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "1.5rem", paddingLeft: 4 }}>
          <Logo size={30} /><span style={{ fontWeight: 600, fontSize: 16 }}>Vendix</span>
        </div>
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {NAV.map(({ to, label, icon: Icon, end }) => (
            <NavLink key={to} to={to} end={end} style={sideNavStyle}>
      ...
      <aside className="mobile-drawer" style={{ position: "fixed", top: 0, left: sidebarOpen ? 0 : "-260px", width: 240, height: "100%", background: "var(--surface)", zIndex: 201, display: "flex", flexDirection: "column", padding: "1.25rem 1rem", transition: "left .25s ease", boxShadow: sidebarOpen ? "4px 0 20px rgba(0,0,0,0.2)" : "none" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Logo size={28} /><span style={{ fontWeight: 600, fontSize: 16 }}>Vendix</span></div>
          <button onClick={() => setSidebarOpen(false)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--text3)" }}>✕</button>
        </div>
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {NAV.map(({ to, label, icon: Icon, end }) => (
            <NavLink key={to} to={to} end={end} style={sideNavStyle} onClick={() => setSidebarOpen(false)}>
              {({ isActive }) => (<><Icon active={isActive} /><span>{label}</span></>)}
            </NavLink>
          ))}
        </nav>
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: "1rem", display: "flex", flexDirection: "column", gap: 10 }}>
          <ThemeToggle />
          <div onClick={() => { navigate("/perfil"); setSidebarOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
            <Avatar user={user} size={30} />
            <div>
              <div style={{ fontSize: 12, fontWeight: 500, color: "var(--text)" }}>{user?.displayName}</div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", minWidth: 0 }}>
        ...
        <header className="mobile-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "var(--surface)", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <Logo size={24} /><span style={{ fontWeight: 600, fontSize: 15 }}>Vendix</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <ThemeToggle />
            <div onClick={() => navigate("/perfil")} style={{ cursor: "pointer" }}><Avatar user={user} size={30} /></div>
          </div>
        </header>
