...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    div style={{ background #fff, border 1px solid var(--gray-100), borderRadius 8, padding 10px 14px, boxShadow var(--shadow-md) }}
      ...
      div style={{ fontSize 12, color var(--gray-500) }}{payload[1].value  0} ventasdiv
    div
  );
}
  ...
  return (
    div style={CARD}
      ...
      div style={{ display flex, justifyContent space-between, alignItems center, marginBottom 12 }}
        div
          div style={{ fontSize 12, color var(--gray-500) }}Ingresos del períododiv
          div style={{ fontSize 20, fontWeight 600, color var(--teal) }}{fmx(totalPeriod)}div
          div style={{ fontSize 11, color var(--gray-500) }}{totalVentas} ventasdiv
        div
        div style={{ display flex, flexDirection column, gap 6, alignItems flex-end }}
          { Period toggle }
          div style={{ display flex, background var(--gray-100), borderRadius 8, padding 3, gap 2 }}
            button style={TAB_STYLE(period === week)} onClick={() = setPeriod(week)}7 díasbutton
            button style={TAB_STYLE(period === month)} onClick={() = setPeriod(month)}6 sembutton
          div
          { Chart type toggle }
          div style={{ display flex, background var(--gray-100), borderRadius 8, padding 3, gap 2 }}
            button style={TAB_STYLE(chartType === area)} onClick={() = setChartType(area)}Áreabutton
            button style={TAB_STYLE(chartType === bar)} onClick={() = setChartType(bar)}Barrasbutton
          div
        div
      div
...
function TopSellers({ sales }) {
  ...
  return (
    div style={{ marginTop 16, paddingTop 14, borderTop 1px solid var(--gray-100) }}
      div style={{ fontSize 12, fontWeight 600, color var(--gray-700), marginBottom 10 }}Top vendedoresdiv
      {sorted.map((s, i) = (
